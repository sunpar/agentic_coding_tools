...
description: "Analyze git history to map security ownership (people<->files), estimate bus factor on sensitive code, and export CSV/JSON for graph analysis. Use only when a user explicitly asks for security-focused ownership or bus-factor analysis grounded in commit history (for example orphaned sensitive code, security maintainers, CODEOWNERS drift, or sensitive hotspots). Not for general maintainer lists or non-security ownership questions."
---
